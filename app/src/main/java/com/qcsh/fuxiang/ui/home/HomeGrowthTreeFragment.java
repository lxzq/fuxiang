...
import com.qcsh.fuxiang.adapter.HomeHuoDongAdapter;
 ...
 */
public class HomeGrowthTreeFragment extends BaseFragment implements XListView.IXListViewListener {
    ...
    private void addComment(final LinearLayout layout, final String content_id) {
        ...
        showProgress();
        ApiClient.post(getActivity(), ApiConfig.GET_COMMENT_LIST, new ApiResponseHandler<CommentEntity>() {
            @Override
...
//                        content.setText(comment_content);
    ...
    private void showPopupWindow(View view, final LinearLayout layout, final GrowthTreeEntity growthTreeEntity) {
        ...
        btnShare.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (growthTreeEntity.getChildren_id().equals(getCurrentUser().getChildId())) {
    ...
    private void initData() {
        showProgress();
        ApiClient.get(getActivity(), ApiConfig.HOME_MY_COLLECT, new ApiResponseHandler<GrowthTreeEntity>() {
        ...
        }, "userId", getCurrentUser().getId(), "type", "1", "page", nextCursor + "");
    }
    ...
    private void deleteItem(String contentId) {
        showProgress();
        ApiClient.post(getActivity(), ApiConfig.GROWTHTREE_DELETE, new ApiResponseHandler<BaseEntity>() {
            @Override
            public void onSuccess(DataEntity entity) {
                closeProgress();
            }
    ...
    private void submitComment(final LinearLayout layout, final EditText edit, final String contentId) {
        ...
        showProgress();
        ApiClient.post(getActivity(), ApiConfig.GROWTHTREE_SENDCOMMENT, new ApiResponseHandler<BaseEntity>() {
            @Override
            public void onSuccess(DataEntity entity) {
                closeProgress();
            }
