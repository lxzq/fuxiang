...
public class AppException extends Exception implements UncaughtExceptionHandler{
	...
	public void saveErrorLog(Exception excp) {
		...
		try {
			...
			if(storageState.equals(Environment.MEDIA_MOUNTED)){
				savePath = Environment.getExternalStorageDirectory().getAbsolutePath() + "/Lvgutou/Log/";
			...
			pw.println("--------------------"+DateFormat.getDateInstance().format(new Date())+"---------------------");
	...
	private boolean handleException(Throwable ex) {
		...
		new Thread() {
			public void run() {
				Looper.prepare();
				//SysBll.sendAppCrashReport(context, crashReport);
				Looper.loop();
			}
